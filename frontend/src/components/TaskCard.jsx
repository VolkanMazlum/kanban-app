...
export default function TaskCard({ task, onDragStart, onEdit, onDelete }) {
  ...
  const uniqueAssignees = Array.from(allAssigneesMap.values());
            ...
            <div style={{ marginTop: 12, borderTop:"1px solid #F3F4F6", paddingTop: 10 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {topics.map(topicName => {
                  const topicPhases = phases.filter(ph => ph.topic === topicName || ph.topic_source === topicName).sort((a,b)=>a.position-b.position);
                                ...
                                {specificAssignees.length > 0 && (
                                   <div style={{ display: "flex" }}>
                                    {specificAssignees.slice(0, 2).map((a, i) => (
                                      <div key={a.id} style={{ marginLeft: i > 0 ? -6 : 0 }} title={a.name}>
                                        <Avatar name={a.name} size={16} idx={i} />
                                      </div>
                                    ))}
                                   </div>
                                )}
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                <div style={{ display:"flex" }}>
                  {uniqueAssignees.slice(0,4).map((a,i) => (
                    <div key={a.id} style={{ marginLeft:i>0?-8:0 }} title={a.name}>
                      <Avatar name={a.name} size={22} idx={i} />
                    </div>
                  ))}
                </div>
                {uniqueAssignees.length===1
                  ? <span style={{ fontSize:11, color:"#6B7280" }}>{uniqueAssignees[0].name}</span>
                  : uniqueAssignees.length>4
                    ? <span style={{ fontSize:11, color:"#6B7280" }}>+{uniqueAssignees.length-4} more</span>
                    : <span style={{ fontSize:11, color:"#6B7280" }}>{uniqueAssignees.length} members</span>
                }
              </div>
            )
          }
